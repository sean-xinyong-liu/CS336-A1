...
def run_rope(
    ...
    """
    rope = RotaryPositionalEmbedding(d_k, theta, max_seq_len, device=in_query_or_key.device)
...
def run_transformer_block(
    ...
    """
    reference_weight = weights["ln1.weight"]
...
def run_transformer_lm(
    ...
    """
    ref_weight = weights["token_embeddings.weight"]
    transformer_lm = TransformerLM(vocab_size, context_length, d_model, num_layers,
                                   num_heads, d_ff, rope_theta, device=ref_weight.device, dtype=ref_weight.dtype)
